...
function logColor(color) {
  console.log(`%c ${color.hex()}`, `background-color: ${color.hex()}`);
}

export default class ImageThemeParser extends React.Component {
  constructor(...args) {
    super(...args);
  ...
  onImageload() {
    ...
    var palletes = this.getColorPalette(image);
    ...
    var mostDominantColor = getMostDominantPrimaryColor(
      WCAGCompliantColorPairs
    );
    var [secondary, tertiary] = WCAGCompliantColorPairs[mostDominantColor];
    /**
     * If there's no tertiary color, use the
     * secondary color as a fallback.
     */
    if (!tertiary) {
      tertiary = secondary;
    }
    this.setState({
      colors: {
        primary: mostDominantColor,
        secondary: secondary.color.rgb(),
        tertiary: tertiary.color.rgb()
      }
    });
  }
  ...
  getColorPalette(image) {
    totalPixelCount = 0;
    RGBToPixelCountMap = {};
    return this.colorThief.getPalette(image).map(color => {
